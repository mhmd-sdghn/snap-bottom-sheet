...
3. **Root files** (clone from the template, apply the rename map — package `snap-bottom-sheet`, repo `mhmd-sdghn/snap-bottom-sheet`, author `Mo Sadeghian`, docs filter `snap-bottom-sheet-docs`):
   ...
   - `.changeset/config.json` — `changelog: ["@changesets/changelog-github", { repo: "mhmd-sdghn/snap-bottom-sheet" }]`, `access: public`, `baseBranch: main`, `ignore: ["snap-bottom-sheet-docs", "playground-react", "playground-next", "@snap-bottom-sheet/spring", "@snap-bottom-sheet/gesture"]`.
