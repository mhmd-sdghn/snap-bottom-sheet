...
- tsdown: `deps.alwaysBundle: ["@snap-bottom-sheet/spring", "@snap-bottom-sheet/gesture"]` (tsdown 0.22 has no `noExternal`) so both TypeScript-source workspace packages are bundled and their types rolled into `dist/index.d.ts`; `deps.neverBundle` keeps react/react-dom external. If the dts pass cannot follow the symlinked `.ts` sources, report before working around it.
