...
Touch-action model used by the sheet (CSS spec: the browser walks from the touched element up to the *nearest ancestor with a default touch behaviour*, i.e. the nearest scroller, and only that segment's `touch-action` values matter): Content panel gets `touch-action: none`; `Sheet.Body` gets `overflow-y: auto; overscroll-behavior: contain; touch-action: pan-x` when the active snap allows scroll, and `overflow: hidden` (no `touch-action`) otherwise, so the walk continues up to the panel and the drag wins. `pan-x`, not the default: **vertical touch scrolling inside Body is driven by the library** (task 19), because the browser cannot hand a scroll back to a drag mid-gesture. `overflow-y: auto` is still what keeps the wheel, the keyboard and the scrollbar native. The arbitration between the two is §3.4 rule 2.
...
2. Drag vs scroll — one gesture, two phases, arbitrated per move on the **incremental** delta `d = clientY - lastClientY` (task 19). The gesture is never cancelled for a scroll reason; the sheet owns it throughout and hands the delta to whichever half should move.
   - `inBody` (the pointerdown target is inside Body) is fixed for the gesture. Mode starts as `scroll` when the list is already scrolled (`scrollTop > 0`), and as `sheet` otherwise.
   - Mode `sheet`: move the sheet by `d` with the usual locks and clamps, but no higher than the **scroll ceiling** — the nearest `scroll: true` snap at or above the current y, when `inBody` and the content would overflow there. The frame that reaches the ceiling clamps to it, hands that snap over to the controller (`enterScrollSnap`: it becomes the active snap and Body is laid out as its scroller) and passes the remainder to mode `scroll`. Nothing to scroll at the ceiling → no ceiling, and the drag carries on to the higher snaps. Outside Body there is no ceiling at all.
   - Mode `scroll`: `body.scrollTop -= d`. Reaching `scrollTop === 0` with the finger still going down passes the remainder back to mode `sheet`; at the end of the list the rest of the delta is dropped (no overscroll, and the sheet does not move up past its ceiling).
   - Release in mode `sheet` → rule 4. Release in mode `scroll` → `core/scroll-momentum.ts`: `scrollTop += v·dt; v *= exp(-dt / 325)`, stopping under 0.02 px/ms or at either end, never bouncing and never moving the sheet. Cancelled by the next pointerdown, a snap change, `close()` and `destroy()`; skipped entirely under reduced motion.
   - `data-dragging` is present while the finger moves the sheet, `data-scrolling` while it moves the content; `SheetState.dragging` covers both, and `onDragStart`/`onDragEnd` fire for the whole gesture either way.
3. Live drag: `y = clamp(y + d, topmostY, viewHeight)`; respect `drag.up === false` / `drag.down === false` of the active snap (no movement in that direction). No rubber-banding above the topmost snap.
...
| 2c | `tasks/06-meta.md` ✅ merged (W2; W3 stalled) | W2 | 0 | `docs: README, CONTRIBUTING, CLAUDE.md, 1.0 changeset` |
